...
# =========================
con = sqltor.connect(host="localhost", user="root", password="Harris@*2005")
...
def setup_db():
    ...
    # Unique appointment number per doctor+date
    try:
        cur.execute("""
...
DOCTOR_PASSWORDS = {
    d[0].lower(): 7001+i for i, d in enumerate(DOCTORS)
}
...
def register_patient():
    ...
    try:
        cur.execute("INSERT INTO appt (idno,name,age,gender,phone,bg) VALUES (%s,%s,%s,%s,%s,%s)",
...
def update_patient_field():
    ...
    if choice=="1":
        ...
        while True:
            try: new=int(input("New age: ").strip()); break
            except: print("Enter numeric age.")
...
def book_appointment():
    ...
    try:
        ...
        if not (1<=idx<=len(depts)):
            print("Invalid choice."); return
    except:
        print("Invalid input."); return
    dept = depts[idx-1]
    candidates = [d for d in DOCTORS if d[1]==dept]
    chosen=random.choice(candidates)
    days_map={"Cardiologist":3,"Rheumatologist":5,"Psychiatrist":3,"Neurologist":6,"Otolaryngologist":4,"MI room":1}
    days=days_map.get(dept, random.choice([1,3,4,5,6]))
    appt_date=datetime.date.today()+datetime.timedelta(days=days)
    appt_no=ensure_unique_appt_no(chosen[0],appt_date)
    cur.execute("INSERT INTO appointments (patient_id,doctor_name,department,room,appt_date,appt_no) VALUES (%s,%s,%s,%s,%s,%s)",
    ...
    while True:
        ...
        if choice=="1":
            while True:
                ...
                if c=="1": register_patient()
                elif c=="2": book_appointment()
                elif c=="3": list_doctors()
                elif c=="4": list_services()
                elif c=="5": update_patient_field()
                elif c=="6": break
                else: print("Invalid choice.")
        elif choice=="2": doctor_login()
        elif choice=="3":
            print("Goodbye."); break
        else:
